...
<template>
  <div class="auth-page">
    ...
    <div class="auth-brand">
      ...
      <p class="brand-desc">创建账号，开启高效自习之旅</p>
      <div class="brand-features">
        <div class="feature-item">
          <span class="feature-dot"></span> 快速注册即刻使用
        </div>
        <div class="feature-item">
          <span class="feature-dot"></span> 预约心仪自习座位
        </div>
        <div class="feature-item">
          <span class="feature-dot"></span> 随时管理个人行程
        </div>
      </div>
    </div>
      ...
      <div class="auth-card">
        ...
        <el-form ref="formRef" :model="form" :rules="rules" size="large" @submit.prevent="handleRegister">
          <el-form-item prop="studentNo">
            <el-input v-model="form.studentNo" placeholder="学号" :prefix-icon="User" class="auth-input" />
          </el-form-item>
          <el-form-item prop="name">
            <el-input v-model="form.name" placeholder="姓名" :prefix-icon="EditPen" class="auth-input" />
          </el-form-item>
          <el-form-item prop="phone">
            <el-input v-model="form.phone" placeholder="手机号" :prefix-icon="Phone" class="auth-input" />
          </el-form-item>
          <el-form-item prop="password">
            <el-input v-model="form.password" type="password" placeholder="密码（8-32位）"
                      :prefix-icon="Lock" show-password class="auth-input" />
          </el-form-item>
        ...
        <div class="auth-footer">
          已有账号？<router-link to="/login" class="footer-link">去登录 →</router-link>
        </div>
      </div>
    </div>
  </div>
...
import { User, Lock, EditPen, Phone } from '@element-plus/icons-vue'

const authStore = useAuthStore()
const loading = ref(false)
const formRef = ref(null)
const form = reactive({ studentNo: '', name: '', phone: '', password: '', confirmPassword: '' })
...
const rules = {
  studentNo: [
    ...
    { min: 6, max: 20, message: '学号长度6-20位', trigger: 'blur' }
  ],
  name: [
    { required: true, message: '请输入姓名', trigger: 'blur' },
    { min: 2, max: 50, message: '姓名长度2-50字', trigger: 'blur' }
  ],
  phone: [
...
async function handleRegister() {
  ...
  if (!valid) return
  loading.value = true
  try { await authStore.register(form) } catch { /* ElMessage已处理 */ }
...
@media (max-width: 860px) {
  ...
  .auth-card { width: 100%; max-width: 460px; padding: 32px 24px; }
}
</style>
