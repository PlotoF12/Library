  ...
  <div class="auth-page">
    ...
    <div class="auth-brand">
      ...
      <div class="brand-icon-wrap">
        <el-icon :size="48" color="#fff"><Avatar /></el-icon>
      </div>
      <h1 class="brand-title">管理控制台</h1>
      <p class="brand-desc">图书馆老师专用管理入口</p>
      <div class="brand-features">
        <div class="feature-item">
          <span class="feature-dot"></span> 座位与预约管理
        </div>
        <div class="feature-item">
          <span class="feature-dot"></span> 数据统计与分析
        </div>
        <div class="feature-item">
          <span class="feature-dot"></span> 系统规则配置
        </div>
      </div>
    </div>
    ...
    <div class="auth-card-wrap">
      <div class="auth-card">
        <div class="card-badge admin-badge">管理员</div>
...
import { User, Lock, Avatar, Back } from '@element-plus/icons-vue'
...
async function handleLogin() {
  ...
  if (!valid) return
  loading.value = true
  try { await authStore.adminLogin(form) } catch { /* ElMessage已处理 */ }
...
.auth-page {
  ...
  gap: 0;
  background: linear-gradient(160deg, #1c1917 0%, #292524 20%, #1e293b 50%, #1e3a5f 80%, #2563eb 100%);
  padding: 40px;
  position: relative;
  overflow: hidden;
}
...
.admin-badge { color: #d97706; background: #fffbeb; }
